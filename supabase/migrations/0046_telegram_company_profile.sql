...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
      'org_id', o.id, 'name', o.name, 'name_en', o.name_en, 'plan', o.plan_code, 'plan_expires_at', o.plan_expires_at,
      ...
      'documents', (select coalesce(jsonb_agg(jsonb_build_object('item_number', i.item_number, 'title', i.title, 'kind', i.data->>'document_kind',
                    ...
                    from public.items i where i.org_id = o.id and i.data ? 'document_kind'))), '[]'::jsonb)
  into result
  from public.organizations o left join public.org_profiles p on p.org_id = o.id
  where o.id in (select public.telegram_user_orgs(p_user_id));
  return result;
end $$;
